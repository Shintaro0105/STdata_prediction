...
class LGnet(nn.Module):
    def __init__(
        ...
        self.il = nn.Linear(2 * input_size + output_size + memory_dim + hidden_size, hidden_size)
        self.fl = nn.Linear(2 * input_size + output_size + memory_dim + hidden_size, hidden_size)
        self.ol = nn.Linear(2 * input_size + output_size + memory_dim + hidden_size, hidden_size)
        self.cl = nn.Linear(2 * input_size + output_size + memory_dim + hidden_size, hidden_size)
    ...
    def step_memory(self, x, x_last_obsv, x_last_obsv_b, x_mean, h, c, mask, delta, delta_b, memory, batch_size):
        delta_z = torch.exp(-torch.max(self.zeros, self.gamma_z_l(delta)))
        delta_z_prime = torch.exp(-torch.max(self.zeros, self.gamma_z_prime_l(delta_b)))
        ...
        # Retrieve global temporal dynamics
        global_dynamics = self.memory[memory, :].unsqueeze(0).repeat(batch_size, 1)
    ...
    def forward(self, input, forecast=False):
        ...
        i = step_size - 1
        Hidden_State_l, Cell_State_l = self.step(
            torch.squeeze(X[:, i : i + 1, :]),
            torch.squeeze(X_last_obsv[:, i : i + 1, :]),
            torch.squeeze(X_last_obsv_b[:, i : i + 1, :]),
            torch.squeeze(self.X_mean[:, i : i + 1, :]),
        ...
        outputs = torch.cat((outputs, self.fc(Hidden_State_l).unsqueeze(1)), 1)
        ...
        if self.output_last:
            ...
            if forecast:
                ...
                for m in range(self.memory_size):
                    Hidden_State_l, Cell_State_l = self.step_memory(
                        torch.squeeze(X[:, i : i + 1, :]),
                        torch.squeeze(X_last_obsv[:, i : i + 1, :]),
                        torch.squeeze(X_last_obsv_b[:, i : i + 1, :]),
                        torch.squeeze(self.X_mean[:, i : i + 1, :]),
                    ...
                    # print(f"Hidden_State_l shape: {Hidden_State_l.shape}")

                    if forecast_by_memory is None:
                        forecast_by_memory = self.fc(Hidden_State_l).unsqueeze(2)
                    else:
                        forecast_by_memory = torch.cat((forecast_by_memory, self.fc(Hidden_State_l).unsqueeze(2)), 2)
