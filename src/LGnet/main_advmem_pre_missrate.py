...
if __name__ == "__main__":
    ...
    output_path = "/workspaces/STdata_prediction/src/LGnet/output/missrate_advmem_BAY_64_8_01.csv"

    results = []
    # Search for the best mask_ones_proportion value
    for mask_ones_proportion in np.arange(1.0, -0.1, -0.1):
        print(f"Testing with mask_ones_proportion = {mask_ones_proportion:.1f}")

        # Prepare dataset and dataloaders
        train_dataloader, valid_dataloader, test_dataloader, max_speed, X_mean = PrepareDataset(
            speed_matrix, BATCH_SIZE=32, masking=True, mask_ones_proportion=mask_ones_proportion, split_num=64
        )
        ...
        lgnet = LGnet_advmem(input_dim, hidden_dim, output_dim, X_mean, memory_size=8, num_layers=1, output_last=True)
        ...
        results.append(
            {
                "Mask_Rate": 1 - mask_ones_proportion,
                "L1_mean": mean_l1,
                "L1_std": std_l1,
                "MAE": MAE_,
                "MAPE": MAPE_,
