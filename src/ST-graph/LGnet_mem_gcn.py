...
class LGnet_mem(nn.Module):
    def __init__(
        self, input_size, hidden_size, output_size, X_mean, memory_size, memory_dim, A, num_layers, output_last=False
    ):
        ...
        self.gcn1 = GCNLayer(input_size, input_size)
        self.gcn2 = GCNLayer(input_size, input_size)
    ...
    def step(self, x, x_last_obsv, x_last_obsv_b, x_mean, h, c, mask, delta, delta_b):
        ...
        X = self.gcn1(local_statistics, self.A)
        X = X.unsqueeze(-1)
