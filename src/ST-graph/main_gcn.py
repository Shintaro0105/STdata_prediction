...
if __name__ == "__main__":
    ...
    if data == "inrix":
        ...
        def create_adjacency_matrix(distance_matrix, threshold):
            ...
            # 対角成分をゼロにする（自分自身との接続は考慮しない）
            np.fill_diagonal(adjacency_matrix, 0)
            return adjacency_matrix

        # 使用例
        threshold = 0.1  # 例としての閾値
        adjacency_matrix = create_adjacency_matrix(distance_matrix, threshold)
    elif data == "BAY":
    ...
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    A = torch.tensor(adjacency_matrix, dtype=torch.float32)
